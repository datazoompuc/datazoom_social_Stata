...
knit: (function(input, encoding) { rmarkdown::render(input, encoding = encoding); md_path <- sub("\\.Rmd$", ".md", input); texto <- paste(readLines(md_path, warn = FALSE), collapse = "\n"); texto <- gsub("<details>\\s*<summary([^>]*)>\\s*(.*?)\\s*</summary>", "<details><summary\\1>\\2</summary>", texto, perl = TRUE); writeLines(texto, md_path) })
---
