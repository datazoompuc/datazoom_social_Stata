...
DataZoom Social Stata `datazoom_social` is a package that enables compatibility with microdata from surveys conducted by IBGE. With this package, it is possible to read all household surveys conducted by IBGE: Demographic Census, National Household Sample Survey (PNAD), Monthly Employment Survey (PME), Household Budget Survey (POF), and Urban Informal Economy Survey (ECINF).
...
| `date(...)`       | Reference date to the harmonized data                                |
...
The available years for the Census are those of 1970, 1980, 1991, 2000, and 2010. In the state option, you can enter the initials of as many states as you want for data reading, for example: `state(AP MS AM)`. There are also the following options:
...
As an example, see the use of this function in the `datazoom_pnadcontinua` program:
    ...
    findfile dict.dta // Finds the dict.dta file saved by the package installation
                      // inthe /ado/ folder and stores the path to it in the r(fn) 
                      //macro.
