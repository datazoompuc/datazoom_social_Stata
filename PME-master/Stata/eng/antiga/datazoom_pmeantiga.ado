...
qui while "`*'" != "" {
	if `1' <= 1999 {
		...
		noi display as input _newline "Extracting `1' - Households"
		loc y = substr("`1'",3,2)
		foreach UF in BA MG PE RJ RS SP {
			findfile pme_1991_2000_dom_en.dct
...
qui while "`*'" != "" {
	if `1' <= 1999 {
		...
		/*Extrai arquivos por UF, separa o ano e empilha em anos*/
		noi display as input _newline "Extracting `1' - Individuals"
		loc y = substr("`1'",3,2)
		foreach UF in BA MG PE RJ RS SP {
			findfile pme_1991_2000_pes_en.dct
		...
		foreach UF in BA MG PR PE RJ RS SP {
			...
			lab var	v201	"number associated with the hh member"
			lab var	v202	"gender"
			lab var	v203	"status inside the household"
			lab var	v204	"status inside the family"
			lab var	v205	"family number"
			lab var	v206	"day of birth"
			lab var	v236	"month of birth"
			lab var	v246	"year of birth"
			lab var	v207	"knows how to read and write"
			lab var	v208	"goes to school"
			lab var	v209	"last grade completed"
			lab var	v210	"education level"
			lab var	v211	"completed the course?"
			lab var	v301	"what did you do during the week?"
			lab var	v302	"had another job /hadn't"
			lab var	v303	"occupation during the week"
			lab var	v304	"activity/business area"
			lab var	v305	"activity area"
			lab var	v306	"position in occupation"
			lab var	v307	"received wage per month, every 15 days"
			lab var	v308	"legally employed?"
			lab var	v309	"monthly income"
			lab var	v339	"income range"
			lab var	v310	"effective weekly hours worked"
			lab var	v311	"monthly income - other jobs"
			lab var	v341	"income range - other jobs"
			lab var	v312	"effective weekly hours worked - other jobs"
			lab var	v313	"took measures during the period?"
			lab var	v314	"took measures before"
			lab var	v315	"which measure was taken?"
			lab var	v931	"when did you take the measure"
			lab var	v316	"when did you take the measure (day)"
			lab var	v346	"when did you take the measure (month)"
			lab var	v356	"when did you take the measure (year)"
			lab var	v366	"when did you take the measure (less or more than 30 days)"
			lab var	v317	"searched for how long (month)"
			lab var	v347	"searched for how long (week)"
			lab var	v318	"has worked before (paid)?"
			lab var	v319	"has worked before (unpaid)?"
			lab var	v320	"for how long hasn't worked? (years)"
			lab var	v350	"for how long hasn't worked? (months)"
			lab var	v360	"for how long hasn't worked? (weeks)"
			lab var	v321	"previous occupation"
			lab var	v322	"previous activity"
			lab var	v323	"previous activity area"
			lab var	v324	"position in previous occupation"
			lab var	v325	"time in last job (years)"
			lab var	v355	"time in last job (month)"
			lab var	v326	"why did you leave last job?"
			lab var	v327	"in the last job, was legally employed?"
			lab var	v328	"after last job, received FGTS?"
			lab var	v256	"calculated age"
...
qui foreach aa in `years' {
    ...
    lab var painel "PME panel"
	lab var npesq "interview number"
	...
	***Branco - Não aplicável
	lab var aest1 "group of years of schooling"
...
if "`nid'"~="" {
	di _newline as result "Databases were saved in `saving'"
	exit
}
...
qui if "`idbas'" != "" {
	noi di as result "Running Basic Identification ..."
	foreach pa in $panels {
		...
		* Parar caso a loop não esteja emparelhando mais
		...
		lab var p201 "new number associated with hh member"
		lab var idind "individual identification"
		drop __* back forw
...
qui if "`idrs'" != "" {
	noi di as result "Running Ribas-Soares Identification ..."
	foreach pa in $panels {
		noi di "Panel `pa'"
		use `PME_Painel`pa'temp'
...
di _newline "Databases were saved in `saving'"
end
