...
/* Deflator */
copy "http://www.econ.puc-rio.br/datazoom/english/deflator_pme.dta" "`saving'/", replace
tempfile deflator
...
while "`*'" != "" {
	...
	foreach mm in 01 02 03 04 05 06 07 08 09 10 11 12 {
		...
		if `1'==2009 &`mm'==07 { 
		di as input "Extracting `mm'/`1' datafile ..."
...
qui if "`idbas'" != "" {
	noi display as result "Running Basic Identification ..."
	foreach pa in $panels {
		noi display as result "Panel `pa'"
		use `PME_Painel`pa'temp', clear
...
qui if "`idrs'" != "" {
	noi display as result "Running Ribas-Soares Identification ..."
	foreach pa in $panels {
		noi display as result "Panel `pa'"
