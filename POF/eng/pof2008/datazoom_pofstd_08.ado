...
qui foreach TR of numlist 6/15 {
	findfile pof2008_tr`TR'_en.dct
...
foreach type in `id' {
	...
	foreach n in 01 02 03 04 05 06 07 08 09 10 11 12 13 14 15 16 {
		...
		di "`k'"
		cap {
			lab var vada`n' "total expenditure in `k'"
	...
	foreach n of numlist 21/28 31/36 41/47 51/54 61/69 610 71/76 81/85 90 ///
		...
		if `n'<100 {
			cap {
				lab var vadd0`n' "total expenditure in `k'"
			...
			cap {
				lab var vadd`n' "total expenditure in `k'"
