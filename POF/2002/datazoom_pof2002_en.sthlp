...
{phang}
{bf:datazoom_pof2002} {hline 2}  Access to STATA format POF 2002-03 databases - Version 1.2
...
{phang}
{cmd:datazoom_pof2002} is composed of three programs.

{phang}
1) datazoom_pofstd_02: run this program to get a POF 2002-03 standard database. It contains
	expenditure/acquisition variables for aggregate items according to IBGE classification.
	For more information, check {help datazoom_pofstd_02}.

{phang}
2) datazoom_pofsel_02: through this program the user can construct his own database by choosing 
	and adding disaggregated items into a basket. For more information, check {help datazoom_pofsel_02}.
	
{phang}
3) datazoom_poftrs_02: run this program to extract STATA format POF 2002-03 databases from
	original files whithout any manipulation. 2002-03 version contains 14 types of register.
	For more information, check {help datazoom_poftrs_02}.
	
{phang}
The first two programs allow the user to choose the level at which the data are to be aggregated: household, 
	consumption unit (family) or individual. Note that some types of register are not compatible with all
	levels.

{phang}
Type "db datazoom_pof2002" on the command window to start.

{title: About the POF 2002-03}
...
income. It has been carried out by IBGE on a five-year basis since 1996 (its first version was published in 1988) and it covers all brazilian territory.
...
The above-cited programs (specially datazoom_pofstd_02 and datazoom_pofsel_02) aim to aid the user's access to this survey.
