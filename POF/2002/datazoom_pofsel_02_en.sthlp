...
{phang}
{bf:datazoom_pofsel_02} {hline 2} Acess to STATA format POF 2002-03 Customized Databases
...
{phang}
{cmd:datazoom_pofsel_02} extracts and constructs personalized STATA format databases (.dta) from original microdata files,
which are not available online (for information on how to acquire original datafiles, see www.ibge.gov.br). This program
is part of POF 2002-03  package, which also contains {help datazoom_poftrs_02} and {help datazoom_pofsel_02} (these programs
...
household, consumption unit (family) or individual.
The same will happen to the income-related variables. The income variables (gross monetary income, gross non-monetary income and total income)
corresponds to the household monthly gross income or to the consumption unity monthly gross income. 
All values are annualized and deflated to January 2003. It is worth noting that each expenditure corresponds
to a determined identification level. So it is not possible to obtain the expenditure of a 
given individual for itens that are associated with the consumption unit
or household. In this case, the user should run the program for each identification level 
...
{phang}
{opt original(string)}  indicates the path of the folder where the original data files are located. All fourteen files must be located in the same
...
{p}  NOTE: (Strongly recommended) In order to facilitate the command use, type "db datazoom_pof1902" on the command window. 
