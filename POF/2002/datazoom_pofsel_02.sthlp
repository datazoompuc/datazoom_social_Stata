...
{phang}
{bf:datazoom_pofsel_02} {hline 2} Acesso à bases personalizadas da POF 2002-03 em formato STATA - Versão 1.2
...
{phang}
{cmd:datazoom_pofsel_02} extrai e contrói bases de dados personalizadas em formato STATA (.dta) a partir dos microdados originais, 
	os quais não são disponibilizados pelo Portal (informações sobre como obter os arquivos originais de dados, consulte o
        site do IBGE www.ibge.gov.br). Este programa é parte do pacote POF 2002-03, que inclui {help datazoom_poftrs_02} e
		{help datazoom_pofstd_02} (todos os programas são independentes uns dos outros).
...
indivíduo. O mesmo vale para variáveis de rendimento. Todos os valores são anualizados e deflacionados para janeiro/2003. 
...
bruto mensal do domicílio ou da unidade de consumo (para registros de unidade de consumo e pessoas). Cada gasto está atrelado 
a um determinado nível de identificação. Assim, em particular, não é possível obter para o indivíduo o gasto com itens associados
à unidade de consumo ou domicílio. Neste caso, execute o programa para cadanível de identificação e utilize o comando {help merge}
...
{p}	OBS: (fortemente recomentado) digite 'db datazoom_pof2002' na janela de comando para utilizar o programa via caixa de diálogo 

{p} Exemplo 1: base final contém gastos anualizados em frutas, farinha de trigo e açúcar refinado para unidades de consumo.

{phang} datazoom_pofsel_02, id(uc) original(c:/mydata) saving(c:/pof) lista(frutas farinha_de_trigo açúcar_refinado)
...
{phang} datazoom_pofsel_02, id(pess) original(c:/mydata) saving(c:/pof) lista(frutas farinha_de_trigo açúcar_refinado)
