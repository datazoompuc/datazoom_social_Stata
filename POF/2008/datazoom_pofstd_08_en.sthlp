...
{phang}
{bf:datazoom_pofstd_08} {hline 2} Access to STATA format POF 2008-09 Standard Databases - Version 1.2
...
{phang}
{cmd:datazoom_pofstd_08} extracts and constructs a standard database of POF 2008-09 from the 
original microdata, which are not available online
(for information on how to acquire original data, see www.ibge.gov.br). This program is part 
of POF 2008-09  package, which also contains {help datazoom_poftrs_08} and {help datazoom_pofsel_08}
(these programs are independent from each other).

{phang} In this standard database, the expenditures on similar items are aggregated to a 
...
different from the predetermined ones (see {help datazoom_pofsel_08} to construct
a personalized basket). All values are annualized and
deflated to January 2009.

{phang} It is worth noting that each expenditure corresponds to a determined identification 
...
{phang} The final database, under any identification level, will contain all variables 
...
{phang}
{opt id(string)}  specifies the identification type for which the expenditures are to be 
computed: household ({opt dom}), 
	consumption unit ({opt uc}) or individual ({opt pess}). As defined by IBGE, consumption
	unit is a set of household residents (which may consist of one, in case
...
{opt original(string)}  indicates the path of the folder where the original data files are
 located. All sixteen files must be located in the same folder so that
...
{phang}  NOTE: In order to facilitate the command use, type "db datazoom_pof2008" on the command window.
