...
{phang}
{bf:datazoom_poftrs_08} {hline 2}  Acesso às bases da POF 2008-09 em formato STATA - Versão 1.2
...
{phang}
{cmd:datazoom_poftrs_08} extrai bases de dados da POF 2008-09 em formato STATA (.dta) a partir dos microdados originais, 
os quais não são disponibilizados pelo Portal (informações sobre como obter os arquivos originais de dados, consulte o
        site do IBGE www.ibge.gov.br). Este programa é parte do pacote POF 2008-09, que inclui {help datazoom_pofstd_08} e
		{help datazoom_pofsel_08} (todos os programas são independentes uns dos outros).
		
{phang} Este programa auxilia o usuário a ter acesso aos 14 tipos de registros existentes na POF 2008-09 sem que haja
...
{phang}
{opt trs(string)}  especifica o(s) tipo(s) de registro(s) que o usuário deseja obter. A versão 2008-09 possui 16 tipos de 
...
{phang} datazoom_poftrs_08, trs(tr1 tr6 tr12) original(c:/mydata) saving(c:/pof)

{phang} A execução do exemplo acima gera três bases de dados, uma para cada tipo de registro especificado 
(1 - Domicílio; 6 - Despesas de 90 dias; 12 - Despesa individual). As três bases serão salvas na pasta
"c:/pof".
