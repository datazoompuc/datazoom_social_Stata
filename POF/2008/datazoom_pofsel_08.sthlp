...
{phang}
{bf:datazoom_pofsel_08} {hline 2} Acesso à bases personalizadas da POF 2008-09 em formato STATA - Versão 1.2
...
{phang}
{cmd:datazoom_pofsel_08} extrai e contrói bases de dados personalizadas em formato STATA (.dta) a partir dos microdados originais, 
	os quais não são disponibilizados pelo Portal (informações sobre como obter os arquivos originais de dados, consulte o
		...
		{help datazoom_pofstd_08} (todos os programas são independentes uns dos outros).
...
indivíduo. O mesmo vale para variáveis de rendimento. Todos os valores são anualizados e deflacionados para janeiro/2009.
As variáveis de rendimento (renda bruta monetária, renda bruta não monetária e renda total) são correspondentes ao rendimento bruto
mensal do domicílio ou da unidade de consumo (para registros de unidade de consumo e pessoas). Cada gasto está atrelado a um determinado 
nível de identificação. Assim, em particular, não é possível obter para o indivíduo o gasto com itens associados à unidade de consumo
ou domicílio. Neste caso, execute o programa para cada nível de identificação e utilize o comando {help merge} para juntar as bases.
...
{p}	OBS: (fortemente recomentado) digite 'db datazoom_pof2008' na janela de comando para utilizar o programa via caixa de diálogo 
...
{phang} datazoom_pofsel_08, id(dom) original(c:/mydata) saving(c:/pof) lista(Feijao Cenoura Roupa_de_crianca Transferencia)
