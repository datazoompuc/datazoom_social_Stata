...
di "`*'"
while "`*'"~="" {
	cap findfile pof2008_`1'.dct
	if _rc==601 findfile pof2008_`1'_en.dct

	if "`1'"=="tr1" {
		qui cap infile using `"`r(fn)'"', using("`original'/T_DOMICILIO_S.txt") clear
	}
	if "`1'"=="tr2" {
		qui cap infile using `"`r(fn)'"', using("`original'/T_MORADOR_S.txt") clear
	}
	if "`1'"=="tr3" {
		qui cap infile using `"`r(fn)'"', using("`original'/T_MORADOR_IMPUT_S.txt") clear
	}
	if "`1'"=="tr4" {
		qui cap infile using `"`r(fn)'"', using("`original'/T_CONDICOES_DE_VIDA_S.txt") clear
	}
	if "`1'"=="tr5" {
		qui cap infile using `"`r(fn)'"', using("`original'/T_INVENTARIO_S.txt") clear
	}
	if "`1'"=="tr6" {
	...
	if "`1'"=="tr16" {
		qui cap infile using `"`r(fn)'"', using("`original'/T_CONSUMO_S.txt") clear
	}
