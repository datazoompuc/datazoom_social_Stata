...
qui foreach TR of numlist 6/15 {  /* WARNING: CONFIRMAR SE ESTA Ã‰ A CESTA PADRÃƒO UTILIZADA PELO IBGE */
	findfile pof2017_tr`TR'.dct
	
	if "`TR'"=="6" {
		infile using `"`r(fn)'"', using("`original'/RENDIMENTO_TRABALHO.txt") clear
	}
	if "`TR'"=="7" {
		infile using `"`r(fn)'"', using("`original'/OUTROS_RENDIMENTOS.txt") clear
	}
	if "`TR'"=="8" {
		infile using `"`r(fn)'"', using("`original'/DOMICILIO.txt") clear
	}
	if "`TR'"=="9" {
		infile using `"`r(fn)'"', using("`original'/INVENTARIO.txt") clear
	}
	if "`TR'"=="10" {
		infile using `"`r(fn)'"', using("`original'/CARACTERISTICAS_DIETA.txt") clear
	}
	if "`TR'"=="11" {
		infile using `"`r(fn)'"', using("`original'/CONSUMO_ALIMENTAR.txt") clear
	}
	if "`TR'"=="12" {
		infile using `"`r(fn)'"', using("`original'/CONDICOES_VIDA.txt") clear
	}
	if "`TR'"=="13" {
		infile using `"`r(fn)'"', using("`original'/RESTRICAO_PRODUTOS_SERVICOS_SAUDE.txt") clear
	}
	if "`TR'"=="14" {
		infile using `"`r(fn)'"', using("`original'/SERVICO_NAO_MONETARIO_POF2.txt") clear
	}
	if "`TR'"=="15" {
		infile using `"`r(fn)'"', using("`original'/SERVICO_NAO_MONETARIO_POF4.txt") clear
	}
...
foreach type in `id' {
	...
	findfile pof2017_tr1.dct        /* WARNING: VERIFICAR SE ESSA BASE Ã‰ A CORRETA */

	qui infile using `"`r(fn)'"', using("`original'/MORADOR.txt") clear
	...
	if "`type'" == "pess" {
		preserve
		findfile pof2017_tr2.dct     /* WARNING: VERIFICAR SE ESSA BASE Ã‰ A CORRETA */
		qui infile using `"`r(fn)'"', using("`original'/DESPESA_COLETIVA.txt") clear
		tempfile tr2
		save `tr2', replace
		restore
