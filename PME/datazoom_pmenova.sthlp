...
{phang}
{bf:datazoom_pmenova} {hline 2} Acesso aos microdados da PME-Nova Metodologia em formato STATA - Versão 1.3
...
{p}	OBS: digite 'db datazoom_pmenova' para utilizar o programa via caixa de diálogo
...
{phang}
{cmd:datazoom_pmenova} extrai e contrói bases de dados da PME-Nova Metodologia em formato STATA (.dta) a partir
...
o período entre 2002 e 2016. 

{phang} Apesar de ser uma pesquisa mensal, este programa não permite a escolha de meses específicos para extração, 
mas somente os anos. Como a pesquisa ainda é publicada pelo IBGE, este programa está em constante atualização. Em geral,
 a última atualização apresenta defasagem de dois meses relativamente ao mês corrente. Isso ocorre devido à 
 divulgação do índice utilizado para deflacionar rendas em pesquisas domiciliares. O usuário deve estar conectado à 
 internet para utilização deste programa, pois o processo de deflacionamento das variáveis monetárias utiliza recursos on line.
  O período de referência é janeiro de 2016.
...
pode ser utilizado para o período de 2002 a 2016. O primeiro mês disponível é março/2002. Não é possível 
...
{phang}  OBS: Recomenda-se a execução do programa por meio da caixa de diálogo. Digite "db datazoom_pmenova" na janela 
...
{phang} Três bases de dados são geradas, uma para cada ano selecionado. Cada base conterá as observações de todos os meses 
...
{phang} Será criada uma base para cada painel da PME pesquisado em 2005 e 2006. Em cada base haverá uma variável 
