...
qui foreach ano in `years' {
	...
	while "`*'" ~= "" {
		...
		loc base = substr("`1'",-8,2) // por exemplo: PNAD81BR.TXT
		loc base = "19`base'"
		if "`base'" == "`ano'" {
			loc base`ano'pes = "`1'"
			loc base`ano'dom = "`1'"
		...
		loc base = substr("`1'",-9,2) // por exemplo: PND83RM4.DAT
		loc base = "19`base'"
		if "`base'" == "`ano'" {
			loc base`ano'pes = "`1'"
			loc base`ano'dom = "`1'"
		...
		loc base = substr("`1'",-7,3) // por exemplo: PNAD1985.DAT
		loc base = "1`base'"
		if "`base'" == "`ano'" {
			loc base`ano'pes = "`1'"
			loc base`ano'dom = "`1'"
		...
		loc base = substr("`1'",-8,3) // por exemplo: PND1989N.DAT
		loc base = "1`base'"
		if "`base'" == "`ano'" {
			loc base`ano'pes = "`1'"
			loc base`ano'dom = "`1'"
		...
		loc base = substr("`1'",-6,2)  // por exemplo: DOM93.DAT
		loc base = "19`base'"
		if "`base'" == "`ano'" {
		...
		/* se formato 1996 */
		loc base = substr("`1'",-8,2) // por exemplo: D96BR.TXT
		loc base = "19`base'"
		if "`base'" == "`ano'" {
			loc tipo = substr("`1'",-9,1)
			loc tipo = lower("`tipo'")
		...
		/* se formato 1997 */
		loc base = substr("`1'",-2,2) // por exemplo: Domicilios97
		loc base = "19`base'"
		if "`base'" == "`ano'" {
			loc tipo = substr("`1'",-9,2)
			loc tipo = lower("`tipo'")
		...
		loc base = substr("`1'",-6,2) // por exemplo: Domicilio98.TXT Pessoa98.TXT
		loc base = "19`base'"
		if "`base'" == "`ano'" {
			loc tipo = substr("`1'",-12,3)
			loc tipo = lower("`tipo'")
		...
		/* se anos 2000 */
		loc base = substr("`1'",-7,3) // por exemplo: PES2006.TXT
		loc base = "2`base'"
		if "`base'" == "`ano'" {
